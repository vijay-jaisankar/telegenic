...
def get_combined_video(input_audio_file,
                       input_video_file,
                       output_video_file,
                       watermark_text=None):
  ...
  if watermark_text is None:
    ...
    logo = (TextClip(watermark_text).set_duration(
      outputclip.duration).resize(height=50).margin(right=8, top=8,
                                                    opacity=0).set_pos(
                                                      ("right", "top")))
    finalclip = CompositeVideoClip([outputclip, logo])

  finalclip.resize( (460,720) )
