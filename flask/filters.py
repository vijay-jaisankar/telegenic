...
#     return None
"""
  Colour changer filter
"""


def apply_colour_change_filter(input_filename, output_filename):
  try:
    clip = VideoFileClip(input_filename)
    new_clip = clip.fl_image(invert_green_blue)
    new_clip_scaled = speedx(new_clip)
    new_clip_scaled.write_videofile(output_filename)

  except Exception as e:
    print(f"Error while processing the colour change filter: {e.with_traceback()}")
    return None
...
def apply_scroll_filter(input_filename, output_filename):
  try:
    clip = VideoFileClip(input_filename)
    new_clip = clip.fl(scroll)
    new_clip_scaled = speedx(new_clip)
    new_clip_scaled.write_videofile(output_filename)
...
def apply_colourx_filter(input_filename, output_filename):
  try:
    clip = VideoFileClip(input_filename)
    new_clip_scaled = invert_colors(clip)
    new_clip_scaled.write_videofile(output_filename)
